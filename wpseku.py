...
from lib import wpcolor, wphttp, wpprint
from modules.attack import wplfi, wpsql, wpxss
...
class WPSeku(object):
    ...
    def Usage(self, ext=False):
        ...
        print(
            "\t%s -t http://localhost/wp-admin/post.php -m GET -q \"post=49&action=edit\" [-x,-s,-l]" % path)
    ...
    def Main(self):
        ...
        if self.xss == True:
            ...
            wpxss.wpxss(self.agent, self.proxy, self.redirect,
                        self.target, self.method, self.query).run()
            sys.exit()
        # sql attack
        if self.sql == True:
            if not self.method:
        ...
        if self.brute == True:
            ...
            wpxmlrpc.wpxmlrpc(self.agent, self.proxy, self.redirect,
                              self.target, self.cookie, self.wordlist, self.user).run()
            sys.exit()
        # discovery
        if self.target:
