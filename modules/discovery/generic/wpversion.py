...
class wpversion:
    ...
    def run(self):
        ...
        try:
            ...
            vers = re.findall('\S+WordPress/(\d+.\d+[.\d+]*)', resp.text)
            if vers:
            ...
            try:
                ...
                resp = self.req.send(url)
                vers = re.findall('\S+?v=(\d+.\d+[.\d+]*)', resp.text)
                if vers:
                    self.printf.plus(
                        'Running WordPress version: %s' % (vers[0]))
                ...
                try:
                    ...
                    resp = self.req.send(url)
                    vers = re.findall(
                        '<generator uri="http://wordpress.org/" version="(\d+\.\d+[\.\d+]*)"', resp.text)
                    if vers:
                        self.printf.plus(
                            'Running WordPress version: %s' % (vers[0]))
                        self.wpvuln().run(vers)
                        ...
                        try:
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    resp = self.req.send(url)
                                    vers = re.findall(
                                        '<meta name="generator" content="WordPress (\d+\.\d+[\.\d+]*)"', resp.text)
                                    if vers:
                                        self.printf.plus(
                                            'Running WordPress version: %s' % (vers[0]))
                                        self.wpvuln().run(vers)
                    ...
                    if j[version[0]]["vulnerabilities"][x]["references"]:
                        for z in range(len(j[version[0]]["vulnerabilities"][x]["references"]["url"])):
                            self.printf.ipri("Reference: %s" % (
                                j[version[0]]["vulnerabilities"][x]["references"]["url"][z]), color="g")
                    self.printf.ipri("Fixed in: %s" % (
                        j[version[0]]["vulnerabilities"][x]["fixed_in"]), color="g")
                    print("")
            else:
