...
class wpsitemap:
    ...
    def run(self):
        self.printf.test("Checking sitemap...")
        try:
            url = self.check.checkurl(self.url, '/sitemap.xml')
            resp = self.req.send(url)
            if resp.text and resp.status_code == 200:
                self.printf.plus('sitemap.xml available under: %s' % (url))
            else:
                self.printf.erro('sitemap.xml not available')
