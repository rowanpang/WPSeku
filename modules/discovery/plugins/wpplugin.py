...
from . import wpchangelog, wplicense, wplisting, wpreadme
...
class wpplugin:
    ...
    def __init__(self, agent, proxy, redirect, url):
        ...
        self.req = wphttp.wphttp(
            agent=agent, proxy=proxy, redirect=redirect, url=url)
        self.wpchangelog = wpchangelog.wpchangelog(
            agent=agent, proxy=proxy, redirect=redirect, url=url)
        self.wplicense = wplicense.wplicense(
            agent=agent, proxy=proxy, redirect=redirect, url=url)
        self.wplisting = wplisting.wplisting(
            agent=agent, proxy=proxy, redirect=redirect, url=url)
        self.wpreadme = wpreadme.wpreadme(
            agent=agent, proxy=proxy, redirect=redirect, url=url)
    ...
    def run(self):
        ...
        try:
            ...
            plugin = re.findall('/wp-content/plugins/(.+?)/', resp.text)
            new = []
            for i in plugin:
                if i not in new:
                    new.append(i)
            if new != []:
                for c in range(len(new)):
                    ...
                    self.printf.ipri('Name: %s - %s' %
                                     (new[c], self.version(new[c])), color="g")
    ...
    def version(self, plugin):
        try:
            ...
            resp = self.req.send(url)
            ver = re.findall(
                '/wp-content/plugins/%s\S+?ver=(\d+.\d+[.\d+]*)' % (plugin), resp.text)
            if len(ver) >= 2:
                return ver[0]
            elif len(ver) == 1:
                return ver[0]
    ...
    def run(self, plugin):
        try:
            ...
            req = requests.get("https://www.wpvulndb.com/api/v2/plugins/" +
                               plugin, headers={'User-agent': 'Mozilla/5.0'}, verify=False)
            j = json.loads(req.content, "utf-8")
            if j[plugin]:
                if j[plugin]["vulnerabilities"]:
                    for x in range(len(j[plugin]["vulnerabilities"])):
                        print("")
                        self.printf.ipri('Title: %s' % (
                            j[plugin]["vulnerabilities"][x]['title']), color="r")
                        if j[plugin]["vulnerabilities"][x]["references"]["url"]:
                            for z in range(len(j[plugin]["vulnerabilities"][x]["references"]["url"])):
                                self.printf.ipri('Reference: %s' % (
                                    j[plugin]["vulnerabilities"][x]["references"]["url"][z]), color="g")
                        self.printf.ipri('Fixed in: %s' % (
                            j[plugin]["vulnerabilities"][x]["fixed_in"]), color="g")
                else:
